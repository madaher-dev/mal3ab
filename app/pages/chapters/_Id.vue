<template>
  <div class="flex flex-col items-center justify-center">
    ...
    >
      ...
      <div
        v-if="!$strapi.user"
        class="flex flex-col items-center justify-center w-6/12 mx-auto mt-12 space-y-3 text-center  bg-mal3ab-background"
      >
      ...
      <div class="flex flex-col space-y-3" v-if="$strapi.user">
        ...
        <transition name="fade">
          <div class="grid gap-8 md:grid-cols-2 xl:grid-cols-3">
            <div class="" v-for="answer in answers" :key="answer.id">
              <div
                class="flex flex-col items-end justify-end p-3 m-3 space-y-2 border rounded-lg  border-mal3ab-green-dark"
                v-if="
                  (answer.users_permissions_user.username =
                    !$strapi.user.username)
                "
              >
                <p class="text-sm font-light text-right rtl">
                  س: {{ answer.question.question }}
                </p>
                <p class="text-lg font-semibold">ج: {{ answer.Answer }}</p>
                <div class="flex flex-row items-center space-x-3">
                  <p>{{ answer.users_permissions_user.username }}</p>
                  <img
                    class="w-10 h-10 rounded-full"
                    v-if="answer.users_permissions_user"
                    :src="
                      getStrapiMedia(
                        answer.users_permissions_user.username.picture.url
                      )
                    "
                    :alt="answer.users_permissions_user.username"
                  />
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async answerQuestion() {
      ...
      // this.answers = await this.$strapi.$answers.find()
    },
  },
  mounted() {
    console.log()
